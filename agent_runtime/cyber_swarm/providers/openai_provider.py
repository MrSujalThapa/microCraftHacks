...
class OpenAIProvider:
    ...
    def complete_json(self, *, system: str, user: str, purpose: str) -> ProviderCallResult:
        ...
        content, usage, repaired = self._complete_with_optional_repair(system, user)
    ...
    def _complete_with_optional_repair(
        ...
        content, usage = self._request(system, user)
        try:
            json.loads(content)
            ...
            repaired_content, repaired_usage = self._request(REPAIR_SYSTEM_PROMPT, repair_user)
            return repaired_content, repaired_usage, True

    def _request(self, system: str, user: str) -> tuple[str, Any]:
        ...
        if not message:
            raise ValueError("OpenAI provider returned an empty response")
